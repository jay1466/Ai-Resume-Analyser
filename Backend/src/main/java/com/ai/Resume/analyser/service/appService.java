...
public class appService {
    ...
    public ResponseEntity<?> extract(String roles, MultipartFile file) throws TikaException, IOException, InterruptedException {
        ...
        if(resultsDto.getScore() !=0){
            ...
            previousTable processedData = new previousTable(uname,resultsDto.getScore(),resultsDto.getAtsoptimizationscore(),roles,file.getBytes(),file.getContentType(),resultsDto.getPros(),resultsDto.getCons(),resultsDto.getSuggestions());
    ...
    public ResponseEntity<?> lastReport() {
        previousTable previousTable = previousTableRepo.findFirstByUserEmailOrderByIdDesc(SecurityContextHolder.getContext().getAuthentication().getName());
        if(previousTable != null){
            // Job from API
            RestTemplate restTemplate = new RestTemplate();
            List<Job> jobs;
            String url = "https://api.adzuna.com/v1/api/jobs/in/search/1?app_id="+applicationId+"&app_key="+applicationApiKey+"&results_per_page=50&what="+previousTable.getRoles()+"&where=tamilnadu&content-type=application/json";
            try{
            ...
            resultsDto.setId(previousTable.getId());
            resultsDto.setRoles(previousTable.getRoles());
            resultsDto.setScore(previousTable.getScore());
            resultsDto.setAtsoptimizationscore(previousTable.getAtsoptimizationscore());
            resultsDto.setPros(previousTable.getPros());
            resultsDto.setCons(previousTable.getCons());
            resultsDto.setSuggestions(previousTable.getSuggestions());
            resultsDto.setJobs(jobs);
    ...
    public ResponseEntity<?> deleteAccount() {
        ...
        try{
            ...
            List<previousTable> userResumes = previousTableRepo.findByUserEmailOrderByIdDesc(uname);
            previousTableRepo.deleteAll(userResumes);
            HttpHeaders headers = new HttpHeaders();
            ResponseCookie cookie = ResponseCookie.from("entrypasstoken","").httpOnly(true).secure(false).sameSite("Strict").maxAge(0).path("/").build();
            headers.add(HttpHeaders.SET_COOKIE,cookie.toString());
    ...
    public ResponseEntity<byte[]> viewResume(Long id) {
        previousTable previousTable = null;
        if (id != null) {
            previousTable = previousTableRepo.findById(id).orElse(null);
        } else {
            previousTable = previousTableRepo.findFirstByUserEmailOrderByIdDesc(SecurityContextHolder.getContext().getAuthentication().getName());
        }

        if(previousTable != null && previousTable.getResumeFile() != null) {
            HttpHeaders headers = new HttpHeaders();
            headers.add(HttpHeaders.CONTENT_TYPE, previousTable.getFileType());
            headers.add(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=resume");
            return new ResponseEntity<>(previousTable.getResumeFile(), headers, HttpStatus.OK);
        }
        return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }

    public ResponseEntity<?> history() {
        List<previousTable> history = previousTableRepo.findByUserEmailOrderByIdDesc(SecurityContextHolder.getContext().getAuthentication().getName());
        history.forEach(item -> item.setResumeFile(null)); // don't send blob to frontend
        return new ResponseEntity<>(history, HttpStatus.OK);
    }

    public ResponseEntity<?> jobs(String roles, int page) {
        RestTemplate restTemplate = new RestTemplate();
        List<Job> jobs;
        String url = "https://api.adzuna.com/v1/api/jobs/in/search/" + page + "?app_id="+applicationId+"&app_key="+applicationApiKey+"&results_per_page=50&what="+roles+"&where=tamilnadu&content-type=application/json";
        try {
            JobSearchResponse response = restTemplate.getForObject(url, JobSearchResponse.class);
            jobs = response.getResults();
        } catch (Exception e) {
