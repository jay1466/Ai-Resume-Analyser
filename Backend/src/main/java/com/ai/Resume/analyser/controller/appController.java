...
public class appController {
    ...
    public ResponseEntity<byte[]> viewResume(@RequestParam(required = false) Long id) {
        return appServices.viewResume(id);
    }

    @GetMapping("/history")
    public ResponseEntity<?> history() {
        return appServices.history();
    }

    @GetMapping("/jobs")
    public ResponseEntity<?> jobs(@RequestParam String roles, @RequestParam(defaultValue = "1") int page) {
        return appServices.jobs(roles, page);
    }
}
